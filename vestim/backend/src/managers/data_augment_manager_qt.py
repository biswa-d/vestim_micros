...
from vestim.backend.src.services import normalization_service
...
class DataAugmentManager(QObject): # Inherit from QObject
    ...
    def apply_augmentations(self,
       ...
       try:
           ...
           train_processed_dir = self.job_service.get_train_folder()
           test_processed_dir = self.job_service.get_test_folder()
    ...
    def get_column_info(self, job_folder: str) -> Dict[str, Dict[str, Any]]:
        ...
        current_job_id = self.job_service.get_job_id()
        if not current_job_id:
            self.logger.error("JobService job_id is not set prior to get_column_info call.")
            raise ValueError("Job context (job_id) not set in JobService for get_column_info.")
    ...
    def get_sample_train_dataframe(self, job_folder: str) -> Optional[pd.DataFrame]:
        ...
        current_job_id = self.job_service.get_job_id()
        if not current_job_id:
            self.logger.error("JobService job_id is not set prior to get_sample_train_dataframe call.")
            return None

        train_processed_dir = self.job_service.get_train_folder()
        if not train_processed_dir or not os.path.isdir(train_processed_dir):
            self.logger.error(f"Train processed directory not found for get_sample_train_dataframe: {train_processed_dir}")
            return None

        train_files = glob.glob(os.path.join(train_processed_dir, "*.csv"))
        if not train_files:
            self.logger.info("No train files found in processed directory for get_sample_train_dataframe.")
            return None
        
        try:
            return pd.read_csv(train_files[0])
        except Exception as e:
            self.logger.error(f"Failed to load sample train dataframe from {train_files[0]}: {e}")
            return None