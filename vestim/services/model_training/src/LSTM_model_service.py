...
class LSTMModel(nn.Module):
    ...
    def apply_pruning(self, dropout=0.2):
        """Apply pruning to the LSTM weights"""
        for name, module in self.named_modules():
            if isinstance(module, nn.LSTM):
                # Prune input-hidden and hidden-hidden weights
                for ln in range(self.num_layers):
                    prune.l1_unstructured(module, name=f'weight_ih_l{ln}', amount=dropout)
                    prune.l1_unstructured(module, name=f'weight_hh_l{ln}', amount=dropout)
