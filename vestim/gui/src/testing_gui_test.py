...
from vestim.gateway.src.hyper_param_manager_test import VEstimHyperParamManager
...
class VEstimTestingGUI:
    ...
    def on_closing(self):
        # If the testing thread is still alive, ensure it is stopped
        if hasattr(self, 'testing_thread') and self.testing_thread.is_alive():
            print("Testing thread is still running. Waiting for it to finish...")
            self.testing_manager.stop_testing()  # Ensure that testing is stopped
            self.testing_thread.join()  # Wait for the testing thread to finish
            print("Testing thread has finished. Closing the window.")
        
        # Properly destroy the window to end the Tkinter main loop
        self.master.destroy()
        print("Window closed. Returning to terminal.")
    ...
    def run_testing(self):
        ...
        # Debugging to ensure the testing manager is starting
        print("Starting the testing manager...")
        self.testing_manager.start_testing(self.queue, self.update_status)

        # Debugging to see if the thread is alive and the queue is filling up
        ...
        while True:
            try:
                ...
                print(f"Got result from queue: {result}")  # Debugging line to check the queue output
                self.add_result_row(result)
            except Empty:
                print("Queue is empty, waiting for results...")  # Debugging to track when the queue is empty
                time.sleep(1)

                # Break the loop if the queue is empty and there are no more tasks
                if not self.testing_manager.testing_thread.is_alive():
                    print("Testing thread has finished.")  # Debugging to check when the thread stops
                    break

        # Debugging to confirm that testing is completed
    ...
    def add_result_row(self, result):
        # Debugging to check the contents of the result
        ...
        # Debugging to ensure the row was added successfully
        ...
        # Debugging to check the row details
        print(f"Creating plot button for model: {model_name}")
